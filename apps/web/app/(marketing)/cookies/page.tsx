  ...
  return (
    <main className="bg-white dark:bg-[#04040a] text-slate-900 dark:text-white min-h-screen pt-32 pb-32 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <p className="mt-3 text-slate-400 dark:text-zinc-500 text-sm">
          Conformément à la réglementation CNIL et à la directive ePrivacy.
        </p>

        <p className="mt-8 text-slate-500 dark:text-zinc-400 leading-relaxed">
          Ce site utilise un nombre minimal de cookies. Nous respectons votre vie privée et ne
          collectons que ce qui est strictement nécessaire au bon fonctionnement de la plateforme ou
          à l&apos;amélioration de votre expérience avec votre consentement explicite.
        </p>
        ...
        <div className="mt-12 space-y-6">
          {cookies.map(({ name, required, desc, examples }) => (
            <div
              key={name}
              className="p-6 rounded-2xl border border-slate-200 dark:border-border bg-slate-50 dark:bg-white/[0.02]"
            >
              <div className="flex items-center justify-between mb-3">
                <h2 className="font-semibold text-white">{name}</h2>
                <span
                  className={`text-[10px] font-semibold px-2.5 py-1 rounded-full ${required ? 'bg-blue-500/15 text-blue-300 border border-blue-500/25' : 'bg-white/5 text-slate-500 dark:text-zinc-400 border border-white/10'}`}
                >
