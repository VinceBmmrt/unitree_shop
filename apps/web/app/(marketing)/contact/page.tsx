    ...
    <main className="bg-white dark:bg-[#04040a] text-slate-900 dark:text-white min-h-screen pt-32 pb-32 px-4 relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto">
        <div className="max-w-2xl mb-16">
          <p className="text-blue-400 font-medium text-sm uppercase tracking-widest mb-3">Contact</p>
          ...
          <p className="mt-4 text-slate-500 dark:text-zinc-400 text-lg">
            Notre équipe commerciale répond sous 24h ouvrées. Pour une demande de devis, utilisez directement notre formulaire dédié.
          </p>
        </div>
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={label} className="flex gap-4 p-5 rounded-2xl border border-slate-200 dark:border-border bg-slate-50 dark:bg-white/[0.02] hover:border-blue-200 dark:border-blue-500/20 hover:bg-slate-100 dark:bg-white/[0.04] transition-all">
                ...
                <div>
                  <p className="text-xs text-slate-400 dark:text-zinc-500 uppercase tracking-widest font-medium">{label}</p>
                  <p className="text-sm font-semibold mt-0.5 text-slate-700 dark:text-zinc-200">{value}</p>
                </div>
              </div>
            ))}
